...
class SearchCommand:
    ...
    def _prepare_recording(self, argv, ifile, ofile):
        ...
        recordings = os.path.join(
            environment.splunk_home, "var", "run", "splunklib.searchcommands", "recordings"
        )
        ...
        recording = os.path.join(
            recordings, self.__class__.__name__ + "-" + repr(time()) + "." + self._metadata.action
        )
        ...
        if dispatch_dir is not None:  # __GETINFO__ action does not include a dispatch_dir
            ...
            make_archive(
                recording + ".dispatch_dir", "gztar", root_dir, base_dir, logger=self.logger
            )
...
SearchMetric = namedtuple(
    "SearchMetric", ("elapsed_seconds", "invocation_count", "input_count", "output_count")
)

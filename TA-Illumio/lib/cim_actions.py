...
class ModularAction(object):
    ...
    def result2stash(self, result, dropexp=default_dropexp, mapexp=default_mapexp, addinfo=False):
        ...
        # get key names
        result_keys = set([x[5:] if x.startswith("__mv_") else x for x in result])
        ...
        if cam_header:
            if self.orig_sid:
                action_idline = self.DEFAULT_IDLINE % (
                    self.get_header_item("orig_action_name", self.action_name, "unknown"),
    ...
    def queuework(self, exit_strategy=None, tries=3, raise_all=False):
        ...
        if supports_workers is not True or not self.action_name or self.action_name == "unknown":
        ...
        try:
            _cam_workers = list(set(json.loads(self.configuration.get("_cam_workers") or "[]")))
        except Exception:
        ...
        # Remove local
        _cam_workers = sorted(list(set(_cam_workers) - set(["local"])))
        ...
        for unused_x in range(0, tries):
            try:
                r, c = rest.simpleRequest(
                    self.CAM_QUEUE_URI, sessionKey=self.session_key, jsonargs=json.dumps(records)
                )
                c = c.decode("utf-8")
                ...
                if r.status == http_client.OK:
                    ...
                    # of each worker
                    for worker in _cam_workers:
                        self.message("Successfully queued action", status="pending", worker=worker)
                    if exit_strategy:
                        sys.exit(0)
                    else:
                        break
            ...
            if exit_strategy:
                ...
                for worker in _cam_workers:
                    self.message("Unable to queue action", status="failure", worker=worker)
